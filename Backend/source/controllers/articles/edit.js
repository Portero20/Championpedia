...
module.exports = {
    edit: (req, res) => {
        try {
            ...
            if (category == "players") {
                query = `UPDATE players SET title="${title}",text="${text}",fullName="${fullName}",nickName="${nickName}",born="${req.body.born}", death=${death != null ? `"${death}"` : null},"${req.body.born}",height="${req.body.height}",weight="${req.body.weight}",nationality="${req.body.nationality}",position="${req.body.position}",team="${req.body.team}",numbers="${req.body.numbers}",goals="${req.body.goals}",debut="${req.body.debut}", retire=${retire != null ? `"${retire}"` : null} WHERE id = ${req.body.id};`
