...
export async function newArticle(category, formData) {
    try {
        ...
        let query = await fetch(`https://championpedia-production.up.railway.app/article/${category}/create`, {
            method: "POST",
            headers: {
...
export async function lastCategories(category, size) {
    try {
        const token = await getToken();
        let query = await fetch(`https://championpedia-production.up.railway.app/article/last/${category}/${size}`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
