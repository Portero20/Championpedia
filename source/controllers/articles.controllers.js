    ...
    One: (req, res) => {
        try {
            ...
            const getRandomTable = () => {
                const tables = ['players', 'trophies', 'teams'];
            ...
            database.query(query, (error, results) => {
                if (error) {
                    ...
                    if (table == "players") {
                        queryImage = `SELECT DISTINCT images.image from images inner join imagesplayers on images.id = imagesplayers.image_id inner JOIN players on imagesplayers.player_id = ${result.id};`
                    } else if (table == "teams") {
                        queryImage = `SELECT DISTINCT images.image from images inner join imagesteams on images.id = imagesteams.image_id inner join teams on imagesteams.team_id = ${result.id};`
                    } else if (table == "trophies") {
                        queryImage = `SELECT DISTINCT images.image from images inner join imagestrophies on images.id = imagestrophies.image_id inner join trophies on imagestrophies.thophy_id = ${result.id};`
                    }

                    database.query(queryImage, function (err, imageResult, filed) {
                        if (err) {
                            return console.log(err)
                        } else {
